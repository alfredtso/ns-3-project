...
#### NetDevice
This corresponds to both the hardware (Network Interface Cards, NICs) and the software drivers of them and also included a queue for packets, so there are two level of queueing in `ns-3`![](https://i.imgur.com/6K0fqMz.png)
#### Node
It is the end hosts by which we can install `Application`, `InternetStack` and `NetDevice` to, similar to our computer. We will be creating two `Node` and connect them with something similar to ethernet cable. ![[Pasted image 20210422165421.png]]
#### Channel
...
### TCP
![](https://i.imgur.com/oi1x2bo.png)
The average delay exhibited dips just after "peaking" and then gradually increase again. This might be due to the `cwnd` adjusting in congestion avoidance. The actual reason might need further investigation as the algorithm for congestion control might be in play here too for there are many choice for it in `ns-3` (Veno, Cubic... etc)

## Average Jitter
![](https://i.imgur.com/UlskOc1.png)
...
- HTTP/3:[https://quicwg.org/base-drafts/draft-ietf-quic-http.html](https://quicwg.org/base-drafts/draft-ietf-quic-http.html)
